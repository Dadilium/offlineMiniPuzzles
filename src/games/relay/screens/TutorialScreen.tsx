...
import { getTutorialDiagrams } from '../tutorialContent';
...
export default function TutorialScreen({ route, navigation }: Props) {
  ...
  const tutorialDiagrams = useMemo(() => getTutorialDiagrams(colors), [colors]);
    ...
    <SafeAreaView style={styles.screen}>
      ...
      <View style={styles.body}>
        ...
        <Text style={styles.stepLabel}>
          {steps.length > 1 ? tc('tutorial.stepOf', { current: stepIndex + 1, total: steps.length }) : t('game.newMechanicLabel')}
        </Text>
