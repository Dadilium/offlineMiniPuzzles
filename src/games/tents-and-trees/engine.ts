 ...
 * must catch. Kuhn's algorithm (DFS augmenting paths); trivial cost at these
 * grid sizes (rows*cols in the dozens, trees in the low tens).
 */
export function hasPerfectMatching(trees: Cell[], tents: Cell[]): boolean {
  ...
  const matchedTree = new Array(tents.length).fill(-1);
  ...
  function tryAugment(treeIndex: number, visited: boolean[]): boolean {
    for (const tentIndex of adjacency[treeIndex]) {
      ...
      if (matchedTree[tentIndex] === -1 || tryAugment(matchedTree[tentIndex], visited)) {
        matchedTree[tentIndex] = treeIndex;
        return true;
      }
    }
    return false;
  ...
  for (let i = 0; i < trees.length; i++) {
    const visited = new Array(tents.length).fill(false);
    if (tryAugment(i, visited)) matched++;
  }
