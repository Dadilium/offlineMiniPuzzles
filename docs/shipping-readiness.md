...
- ~~`expo-updates` (EAS Update/OTA) — deliberately skipped for now.~~ **Done (2026-08-08).** `expo-updates@~0.25.28` installed. Went with **background-download-apply-next-launch**, not block-splash-on-fetch: `src/startup/updates.ts` (`checkForUpdates`) calls `Updates.checkForUpdateAsync()`/`fetchUpdateAsync()` fire-and-forget from `App.tsx`, in parallel with (not inside) `runStartupTasks` — the splash gate is untouched, still only waits on ad consent/SDK init + the 2s floor. A fetched update is never force-applied via `reloadAsync()`; it only takes effect the next time the app is cold-started naturally, so an in-progress puzzle is never interrupted. `app.json`: `runtimeVersion.policy: "appVersion"`, `updates.checkAutomatically: "ON_ERROR_RECOVERY"` (disables the native auto-check-on-launch so there's exactly one check path, the manual JS one), `updates.fallbackToCacheTimeout: 0` (never blocks launch waiting on network). `eas.json` build profiles now each set `channel` (`development`/`preview`/`production`) matching their `environment`. Publish via `npm run update:<channel>`. The Sentry `ExpoUpdatesListener` integration filter (added when there was no update pipeline to hook into) is removed — it's included in `defaults` again now that `expo-updates` is installed.
